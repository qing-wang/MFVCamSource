                            ...
                            unsigned char UV0[])
{
    ...
    {
        // Qing: modified for RGB32, and swap R/B components
        //Load R,G,B elements from first row (and convert to float).
        float r00 = (float)I0[x*4 + 2];
        float g00 = (float)I0[x*4 + 1];
        float b00 = (float)I0[x*4 + 0];

        //Load next R,G,B elements from first row (and convert to float).
        float r01 = (float)I0[x*4 + 6];
        float g01 = (float)I0[x*4 + 5];
        float b01 = (float)I0[x*4 + 4];

        //Load R,G,B elements from second row (and convert to float).
        float r10 = (float)I1[x*4 + 2];
        float g10 = (float)I1[x*4 + 1];
        float b10 = (float)I1[x*4 + 0];

        //Load next R,G,B elements from second row (and convert to float).
        float r11 = (float)I1[x*4 + 6];
        float g11 = (float)I1[x*4 + 5];
        float b11 = (float)I1[x*4 + 4];
              ...
              unsigned char J[])
{
    ...
    {
        I0 = &I[y*image_width*4];		//Input row width is image_width*3 bytes (each pixel is R,G,B).
        I1 = &I[(y+1)*image_width*4];
